...
# Method 2
def repeatedNumber(A):
        sum_of_given_array = 0
        sum_of_squares = 0
        
        for i in Arr:
            sum_of_given_array += i
            sum_of_squares += i*i 
        
        sum_of_all = (len(Arr)*(len(Arr)+1)) // 2
        sum_of_all_squares = sum_of_all * ((2* len(Arr) + 1) // 3) 

        C = sum_of_all - sum_of_given_array 
        D = (sum_of_all_squares - sum_of_squares)  // C 
